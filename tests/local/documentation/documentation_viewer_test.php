...
namespace local_taskflow\local\documentation;

/**
 * Tests for {@see \local_taskflow\local\documentation\documentation_viewer}.
 *
 * @package    local_taskflow
 * @category   test
 * @copyright  2026 Wunderbyte GmbH <info@wunderbyte.at>
 * @license    https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @covers \local_taskflow\local\documentation\documentation_viewer
 */
final class documentation_viewer_test extends \advanced_testcase {
    ...
    public static function normalize_path_provider(): array {
        return [
            ...
            'nested file' => ['user/rules/README.md', 'user/rules/README.md'],
            'current dir segments' => ['./user/./README.md', 'user/README.md'],
            'parent inside root' => ['user/rules/../README.md', 'user/README.md'],
            'multiple slashes' => ['user//README.md', 'user/README.md'],
            'backslashes' => ['user\\README.md', 'user/README.md'],
            'escapes root' => ['../version.php', null],
            'escapes root deeply' => ['user/../../settings.php', null],
            'escapes and returns' => ['../taskflow/docs/README.md', null],
        ];
    }
    ...
    public function test_resolve(): void {
        ...
        $resolved = documentation_viewer::resolve('user/README.md');
        $this->assertSame(documentation_viewer::TYPE_MARKDOWN, $resolved['type']);
        $this->assertSame(realpath(documentation_viewer::docs_root() . '/user/README.md'), $resolved['path']);

        foreach (
            [
                'traversal' => '../version.php',
                'deep traversal' => 'user/../../settings.php',
                'absolute path' => '/etc/passwd',
                'wrong extension' => 'README.txt',
                'missing file' => 'user/doesnotexist.md',
                'directory' => 'user',
                'php file' => '../settings.php',
            ] as $label => $badpath
        ) {
            try {
    ...
    /**
     * Rendering returns the first heading as title and produces HTML.
     */
    public function test_render_page(): void {
        $page = documentation_viewer::render_page('README.md');
        $this->assertNotEmpty($page['title']);
        $this->assertStringContainsString('<h1', $page['html']);

        $page = documentation_viewer::render_page('user/README.md');
        $this->assertNotEmpty($page['title']);
        $this->assertStringContainsString('<h1', $page['html']);
    }

    /**
     * Rendering a non-markdown path throws.
     */
    public function test_render_page_rejects_non_markdown(): void {
        $this->expectException(\moodle_exception::class);
        documentation_viewer::render_page('user');
    }
    ...
    public function test_rewrite_html(): void {
        ...
        $rewritten = documentation_viewer::rewrite_html($html, 'user/rules');

        $this->assertStringContainsString('local/taskflow/documentation.php?file=user%2FREADME.md', $rewritten);
        $this->assertStringContainsString('documentation.php?file=user%2Frules%2Frule-types.md', $rewritten);
        $this->assertStringContainsString('rule-types.md#anchor', $rewritten);
        $this->assertStringContainsString('documentation.php?file=developer%2FREADME.md', $rewritten);
        ...
        $this->assertStringContainsString('documentation.php?file=user%2Frules%2Fpix%2Foverview.png', $rewritten);
        $this->assertStringContainsString('img class="img-fluid"', $rewritten);
        $this->assertStringNotContainsString('<table>', documentation_viewer::rewrite_html('<table><tr></tr></table>', ''));
    }
}
