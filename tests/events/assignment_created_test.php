...
final class assignment_created_test extends advanced_testcase {
     ...
     * @covers \local_taskflow\events\assignment_created
     */
    public function test_assignment_created(): void {
        global $DB;
        ...
        $this->runAdhocTasks();
        $storedevents = $sink->get_events();
        $event = reset($storedevents);
        $this->assertInstanceOf(assignment_created::class, $event);
        $this->assertEquals($event->get_name(), 'Assignment was created');
        $this->assertEquals(
            "An assignment {$event->objectid} was created for the user ({$user->id}) with the ruleid ({$id}).",
