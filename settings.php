...
if ($hassiteconfig) {
    ...
    if ($ADMIN->fulltree) {
        ...
        $subpluginname = get_config($componentname, 'external_api_option');
            $subpluginpath = core_component::get_plugin_directory('taskflowadapter', $subpluginname);
            $formfilepath = $subpluginpath . '/classes/form/internal_communication_form.php';
        if (!empty($subpluginpath) && file_exists($formfilepath)) {
