 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class due_date_with_extension implements placeholders_interface {
    ...
    public function render(&$message) {
        $placeholdertarget = "<due_date_with_extension>";
    ...
    private function get_replacement($messageid) {
        $rule = $this->rule;
        $rulesjson = $rule->get_rulesjson();
        $ruledata = json_decode($rulesjson);
        return date('d.m.Y', (int) $this->assignment->duedate + (int) $ruledata->rulejson->rule->extensionperiod);
    }
    /**
     * Factory for the organisational units
     * @param int $ruleid
