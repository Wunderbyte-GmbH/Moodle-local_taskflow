 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class opentargets implements placeholders_interface {
    ...
    private function get_replacement($messageid) {
        $targets = [];
        $assignetmentstargets = json_decode($this->assignment->targets);
        foreach ($assignetmentstargets as $target) {
            if ($target->completionstatus == '0') {
