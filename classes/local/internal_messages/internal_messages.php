<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Unit class to manage users.
 *
 * @package local_taskflow
 * @author Jacob Viertel
 * @copyright 2025 Wunderbyte GmbH
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace local_taskflow\local\internal_messages;

use stdClass;

/**
 * Class unit for assignment specific messages
 * @author Jacob Viertel
 * @copyright 2025 Wunderbyte GmbH
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class internal_messages {
    /** @var int assignmentid. */
    public int $assignmentid;

    /** @var int assignmentid. */
    const TABLENAME = 'local_taskflow_int_com';

    /**
     * Class to handle all assignment messages
     * @param int $assignmentid
     * @return void
     */
    public function __construct($assignmentid) {
        $this->assignmentid = $assignmentid;
        return;
    }

    /**
     * Set all user assignemnts
     * @return array
     */
    public function get_all_assignment_messages(): array {
        global $DB;

        $sql = "
            SELECT m.*, u.firstname, u.lastname
            FROM {" . self::TABLENAME . "} m
        LEFT JOIN {user} u ON u.id = m.usermodified
            WHERE m.assignmentid = :assignmentid
        ORDER BY m.timecreated ASC
        ";

        $params = ['assignmentid' => $this->assignmentid];

        return $DB->get_records_sql($sql, $params);
    }

    /**
     * Set all user assignemnts
     * @param string $message
     * @return void
     */
    public function set_new_assignment_message($messagetext): void {
        global $USER, $DB;
        $message = new stdClass();
        $message->assignmentid = $this->assignmentid;
        $message->message = $messagetext;
        $message->usermodified = $USER->id;
        $message->timemodified = time();
        $message->timecreated = time();
        $DB->insert_record(self::TABLENAME, $message);
    }
}
