...
class standard_assignment implements assignments_interface {
     ...
     * @return array
     */
    public static function get_all_user_assignments_with_active_status($userid, $active, $status) {
        global $DB;
        return $DB->get_records(
            self::TABLE,
            [
     ...
     * @return array
     */
    public static function get_all_revalid_unit_user_assignments($userid, $unitids) {
        global $DB;
        [$insql, $inparams] = $DB->get_in_or_equal($unitids, SQL_PARAMS_NAMED);
        $params = array_merge(['userid' => $userid], $inparams);
